...
<!-- generated by tools/build_docs.py - edit that, not this -->
...
| `PB015` | warning | high | low | no | Repository material ships inside the skill |
...
### PB015 - Repository material ships inside the skill

**Why it matters.** SKILL.md sits at the root of its git repository, beside tests, docs, examples or CI. An installer copies the skill's folder, which here is the whole repository: the test corpus and the documentation land in every user's skills directory, and every marketplace scanner reads them as the skill. A fixture of an attack that tests a scanner reads, to the next scanner, as an attack.

**Fix.** Move the skill into `skills/<name>/` and keep the repository's own material beside it. `.sqsignore` does not help: it hides a directory from this suite, not from the installer or anybody else's scanner.
